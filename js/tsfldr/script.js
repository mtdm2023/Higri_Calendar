$(document).ready(function () {
  ...
  // here we update Output string values of current monthDate; 
    Fill_Prev_AND_Next_Months_Days(RecordsArray_Counter, records[RecordsArray_Counter]["dow"],records[RecordsArray_Counter]["hmnlen"]);
  ...
  function FirstFillHegryData() {
    const notDuplicatedYears_ARR = records.filter((record, index, arr) => {return index === 0 || record["hyr"] !== arr[index - 1]["hyr"];});
    ...
    $.TsHMonthARNames.forEach((M_name)=>{
      HegryMonthList.append(`<li><a class="month_item dropdown-item" href="#" role="menuitem">${M_name}</a></li>`);
      })
    const currentDate = getCurrentDate();
    const dateNow_array = currentDate.split('/');
    RecordsArray_Counter = records.findIndex(
      (record) =>record.greg_date.split('/')[2] === dateNow_array[2] &&record.greg_date.split('/')[0].padStart(2, '0') === dateNow_array[1]);
      Base_RecordsArray_Counter = RecordsArray_Counter;
    CurrentSelectedYear.text(records[RecordsArray_Counter]['hyr']);     
    FillHegryDate(records[RecordsArray_Counter]['hyr'], records[RecordsArray_Counter]['hmn'], records[RecordsArray_Counter]['hmnlen'], records[RecordsArray_Counter]['dow']);
    // here we get the current hegry day that is corresponding to current gregorian date
    RefreshCalender();
    highlightOnCurrentHegryDay();

    Get_HegryData_Gregdate_According_2_SelectedDay(null,$($('.CalenderDay')[Highlighted_day]).text(),  records[RecordsArray_Counter]['hmn'] ,  records[RecordsArray_Counter]['hyr'] , $.TsWeakDayNames[get_indexof_selectedDay($($('.CalenderDay')[Highlighted_day]).text())],isToltip);
  }
  ...
  function FillHegryDate(Hyear, HegryMonth, HegryMonthLength, startDay) {
    ...
    Fill_Prev_AND_Next_Months_Days(RecordsArray_Counter,startDay,HegryMonthLength);
  }
  ...
  function RefreshCalender()
  {
    ...
    lastMonthText.text(
        $.TsHMonthARNames[ records[RecordsArray_Counter - 1 != -1 ? RecordsArray_Counter - 1: RecordsArray_Counter]["hmn"] - 1]);
    Fill_Prev_AND_Next_Months_Days(RecordsArray_Counter, records[RecordsArray_Counter]["dow"],records[RecordsArray_Counter]["hmnlen"]);
 ...
 function next_prevCalnderBtnsHandler()
  {
    previousMonth.on("click", (e) => {  
      ...
      if (nextIndex < records.length) {
        nextMonthText.text(
            $.TsHMonthARNames[records[nextIndex]["hmn"] - 1]
        );
      } else {
      ...
      if (prevIndex >= 0) {
        lastMonthText.text(
            $.TsHMonthARNames[records[prevIndex]["hmn"] - 1]
        );
      } else {
  ...
  function Year_Month_ClickHandler() {
    ...
    year.on("click", function (e) {
      ...
      fill_selectedYearMonth_in_Calender(CurrentSelectedYear.text(),CurrentSelectedMonth.text());
    });
  ...
  function fill_selectedYearMonth_in_Calender (currentSelectedYear , CurrentSelectedMonth) { 
    let index = records.findIndex((value) =>value['hyr'] == currentSelectedYear &&$.TsHMonthARNames[value['hmn'] - 1] == CurrentSelectedMonth);
  ...
  function Calender_Day_click_handler () {
     $('.CalenderDay').on("click", (e)=>{
      $('.CalenderDay').removeClass("highlight");
        ...
        Get_HegryData_Gregdate_According_2_SelectedDay(e.target,$(e.target).text() ,  $.TsFtrNow[0]['_HMM_'] ,  $.TsFtrNow[0]['_HY4_'] , records[RecordsArray_Counter]["dow"],isToltip);
     })
  ...
  function Get_HegryData_Gregdate_According_2_SelectedDay(targetElement,SelectedDay, CurrentMonth, currentYear, startDay,isToltip) 
  {
     // Filter the records to get the current Gregorian date for the Hijri date
     let CurrentGregDate = records.filter((record) => record["hmn"] == CurrentMonth && record['hyr'] == currentYear);
               
     // Calculate the corresponding Gregorian date
     const correspondingGregDate = addDaysToDate(CurrentGregDate[0]["greg_date"], parseInt(SelectedDay, 10) - 1);
  ...
  function get_indexof_selectedDay(SelectedDay) {
      ...
      $('.calender_days').find('tr').each(function (rowIndex, tr) {
        $(tr).find('td').each(function (cellIndex, td) {
          if (SelectedDay === $(td).text().trim() && !$(td).hasClass('inactive')) {
  ...
  function Fill_Prev_AND_Next_Months_Days(Current_DataArray_counter, CurrentMonthStartDay) {
    // Get the current record
    const currentRecord = records[Current_DataArray_counter];
  
    // Previous month days
    const prevRecord = Current_DataArray_counter > 0 ? records[Current_DataArray_counter - 1] : null;
  ...
  async function readData() {
    try {
      ...
      recordss = $.CalCSVData.map((row) => ({
        ...
        hmn: row["hmn"],
        hmnlen: row["hmnlen"],
        greg_date: row["greg_date"]? new Date(row["greg_date"]).toLocaleDateString(): null,
        dow: row["dow"],
      }));
  ...
  {  
    $('html').css('font-family', $.TsFontfamily.trim() ==="" ? 'inherit' :  $.TsFontfamily  );
  }
